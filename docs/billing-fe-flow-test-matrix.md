...
| Workspace Owner/Admin | Pay online open invoice | `/{workspaceSlug}/billing` | `POST /invoices/{id}/checkout` | Open invoice redirects to Stripe checkout URL. | FE action exists. Real Stripe checkout requires `sk_test_...` + Stripe CLI + gateway; mock checkout fallback has been removed from backend. | Blocked |
| Workspace Owner/Admin | Verify payment success | `/workspace/payment/success?session_id=...` | `GET /payments/checkout-session/{sessionId}` | Page verifies checkout session; if verification is pending it displays pending state instead of assuming paid. | FE action exists. Real `cs_test_...` path still needs Stripe CLI/gateway; mock session verification has been removed from backend. | Blocked |
...
| --- | --- | --- | --- |
| Stripe happy path | `sk_test_...`, Stripe CLI `whsec_...`, frontend `localhost:3000`, gateway `localhost:5200` | Checkout with `4242 4242 4242 4242` returns success, invoice becomes paid, one payment row exists for `cs_test_...`. | Blocked for real Stripe: Docker/gateway unhealthy and no Stripe CLI listener. Mock fallback has been removed. |
...
| Duplicate webhook | Stripe CLI replay or backend test | Duplicate event does not create duplicate payment or invoice settlement. | Real webhook replay blocked by Stripe CLI/gateway. |
...
- Direct BillingService API smoke: sales inquiry create/duplicate/validation, admin inquiry list/status/link/convert, pricing config read, rate card read, invoice checkout, checkout-session verification, repeated verification idempotency.
