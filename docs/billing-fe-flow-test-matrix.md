...
| Visitor/Lead | Select volume, features, languages | `/` pricing section | none | Estimated credits/hour and credits/cycle update from selected intent. | Static source contract confirms calculator intent and estimate wiring. | Pass |
| Visitor/Lead | Submit Request pricing | `/` contact form | `POST /sales-inquiries` | Creates a real sales inquiry with consent, source `landing_pricing`, structured estimate, features, languages, company, and email. | Direct BillingService smoke returned `201` and inserted a real row; invalid email returned `400`; duplicate email/company returned the existing row and DB count stayed `1`. Gateway E2E remains blocked. | Pass |
| Visitor/Lead | Preserve intent for signup | `/` | `sessionStorage warptalk:pricing-intent` | Register/workspace create can prefill from latest pricing intent; DB remains admin source of truth. | Landing persists pricing intent; register and workspace create read it. | Pass |
| Registering User | Register from pricing intent | `/register` | auth register APIs | Form can read pricing intent context from session storage. | Register reads `warptalk:sales-package-intent` and falls back to `warptalk:pricing-intent`; route smoke pass. | Pass |
...
| Workspace Owner/Admin | Start 14-day Enterprise trial | `/{workspaceSlug}/billing` | `POST /subscriptions/trial` | Trial starts only for eligible workspace and refreshes subscription, balance, invoices. | FE action and backend tests for trial entity pass; runtime API not verified. | Blocked |
...
| System Admin | Simulate cycle close | `/billing/workspace/{id}` | `POST /subscriptions/workspace/{id}/simulate-cycle-close` | Creates open NET invoice for cycle settlement; does not run trial invoices. | Direct BillingService smoke returned `200` and created an `open` invoice with subtotal 1,900,000 VND, tax 190,000 VND, due date +15 days. Required DB migration alignment for legacy invoice columns. | Pass |
...
- Direct BillingService API smoke: sales inquiry create/duplicate/validation, admin inquiry list/status/link/convert, pricing config read, rate card read, simulate cycle close, invoice checkout mock, checkout-session verification, repeated verification idempotency.
