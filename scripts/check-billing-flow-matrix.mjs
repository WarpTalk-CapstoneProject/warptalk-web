...
const checks = [
  ...
  ["workspace billing contract request sends monthly billing frequency", files.workspaceBilling, 'billingFrequency: "Monthly"'],
  ...
  ["billing transaction type matches backend consume enum", files.billingTypes, "'consume' | 'top_up'"],
  ["global billing ledger filters backend consume transactions", files.internalBilling, 'SelectItem value="consume"'],
  ["workspace billing ledger filters backend consume transactions", files.internalWorkspace, 'SelectItem value="consume"'],
...
const forbidden = [
  ...
  ["billing service must not expose manual cycle close", files.billingService, "simulateCycleClose:"],
  ["system admin workspace page must not expose manual cycle close", files.internalWorkspace, "Manual Cycle Close"],
  ["system admin workspace page must not call cycle close endpoint", files.internalWorkspace, "billingService.simulateCycleClose"],
  ...
  ["billing types must not use legacy consumption enum", files.billingTypes, "'consumption'"],
  ["global billing ledger must not filter legacy consumption type", files.internalBilling, 'SelectItem value="consumption"'],
  ["workspace billing ledger must not filter legacy consumption type", files.internalWorkspace, 'SelectItem value="consumption"'],
