...
import { foldWireStatus } from "./wire-status.ts";
...
export function resolveHistoryStatus(status: string | null | undefined): HistoryRoomStatus {
  return foldWireStatus(status) === "cancelled" ? "cancelled" : "ended";
}
...
export function resolveArtifactStatus(status: string | null | undefined): ArtifactStatus {
  const normalized = foldWireStatus(status);
  if ((ARTIFACT_STATUSES as string[]).includes(normalized)) {
    return normalized as ArtifactStatus;
  }
