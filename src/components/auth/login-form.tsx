...
/**
 * The sign-in screen, shared by the web (/login) and the desktop app's entry route (/desktop-login).
 *
 * WHY ONE SCREEN
 *   /desktop-login used to be a 600-line copy with its own dark design — a glass card over line
 *   waves, "Sign in" instead of "Log in or sign up", a Back button to a welcome splash — while the
 *   web page moved on to the white halftone design. The same account signed in through two screens
 *   that looked like two products, and every fix to one (post-login destination, unverified
 *   accounts, invitation tokens) had to be remembered in the other. Both routes now render this.
 *
 *   The desktop shell loads the web app in its own window, so everything here works there too: the
 *   links stay on the trusted origin, and Google sign-in opens the same popup.
 */
...
function LoginForm() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      toast.error(getErrorMessage(err, t("toasts.loginFailed")));
    }
  };
...
/** The whole screen, with the loading state `useSearchParams` needs a Suspense boundary for. */
