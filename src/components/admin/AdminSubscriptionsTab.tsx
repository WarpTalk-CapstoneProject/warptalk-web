...
export function AdminSubscriptionsTab() {
  ...
  const cancelMutation = useMutation({
    mutationFn: (workspaceId: string) =>
      billingService.cancelSubscription(workspaceId, "Admin force cancel"),
    onSuccess: () => {
  ...
  const filteredSubscriptions = useMemo(() => {
    return subscriptions.filter((sub) => {
      ...
      const matchPlan =
        planFilter !== "ALL"
          ? sub.planName?.toLowerCase().includes(planFilter.toLowerCase())
          : true;
            ...
            {isLoading ? (
              ...
              paginatedSubscriptions.map((sub) => (
                ...
                >
                  <TableCell>
                    <Link
                      href={`/billing/workspace/${sub.workspaceId}`}
                        ...
                        <DialogContent className="bg-surface-1 border-hairline rounded-xl">
                          <DialogHeader>
                            ...
                            <DialogDescription className="text-sm text-muted-foreground mt-2">
                              ...
                              for Workspace{" "}
                              <strong>
                                {sub.workspaceId?.substring(0, 8) || "Unknown"}
                              </strong>
        ...
        <p className="text-xs text-muted-foreground">
          {data ? (
            <>
              ...
              of <strong>{displayTotalCount}</strong> subscriptions
            </>
          ) : (
            "Loading..."
          )}
