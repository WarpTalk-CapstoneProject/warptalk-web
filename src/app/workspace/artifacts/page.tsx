...
export default function WorkspaceArtifactsPage() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-2 overflow-hidden pb-1">
      ...
      <div className="grid min-h-0 flex-1 gap-2 xl:grid-cols-[300px_minmax(0,1fr)_280px]">
        <Card className="min-h-0 overflow-hidden rounded-3xl border-white/70 bg-white/88 shadow-[0_18px_45px_rgba(15,23,42,0.08)]">
          ...
          <CardContent className="max-h-[560px] space-y-2 overflow-y-auto p-2.5">
            {visibleArtifacts.map((artifact) => (
              <button key={artifact.id} onClick={() => setSelectedId(artifact.id)} className={`w-full rounded-2xl border p-2.5 text-left transition ${selected.id === artifact.id ? "border-neutral-950 bg-neutral-950 text-white" : "bg-white hover:border-neutral-400"}`}>
        ...
        <Card className="min-h-0 overflow-hidden rounded-3xl border-white/70 bg-white/90 shadow-[0_18px_45px_rgba(15,23,42,0.08)]">
          <CardHeader className="flex-row items-center justify-between space-y-0 border-b p-3">
          ...
          <CardContent className="flex h-[calc(100%-61px)] min-h-[420px] flex-col p-3">
            <div className="mb-2 flex items-center justify-between rounded-2xl bg-neutral-100 px-3 py-2">
            ...
            <Textarea
              ...
              className="min-h-0 flex-1 resize-none rounded-2xl bg-white p-3 font-mono text-sm leading-6"
            />
          </CardContent>
        </Card>
        ...
        <div className="space-y-2">
          <Card className="rounded-3xl border-white/70 bg-white/88">
            <CardHeader className="px-4 py-3"><CardTitle className="flex items-center gap-2 text-base"><LockKeyhole className="h-5 w-5" />Document access</CardTitle><p className="text-xs text-muted-foreground">Choose who can open or download this meeting.</p></CardHeader>
            <CardContent className="space-y-1.5 px-4 pb-3">
              {permissionOptions.map((permission) => (
                <label key={permission} className="flex cursor-pointer items-center gap-2 rounded-xl border bg-white px-2.5 py-2 text-sm">
