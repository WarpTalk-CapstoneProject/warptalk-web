    ...
    <div className="flex min-h-full flex-col gap-3 pb-2">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Revenue, capacity, meetings, and AI consumption across the workspace.</p>
        </div>
      ...
      <section className="grid min-h-0 flex-1 gap-3 xl:grid-cols-[minmax(0,1.55fr)_minmax(300px,.8fr)]">
        ...
        <Card className="rounded-3xl border-white/70 bg-white/86 shadow-[0_18px_45px_rgba(15,23,42,0.08)]">
          ...
          <CardContent className="space-y-3">
            {billingRows.map((row) => (
              <div key={row.item} className="flex items-center justify-between rounded-2xl border bg-white/80 p-3">
                <div>
      ...
      <section className="grid gap-3 lg:grid-cols-[minmax(0,1.5fr)_minmax(280px,.65fr)]">
        <Card className="rounded-3xl border-white/70 bg-white/86">
          ...
          <CardContent className="grid gap-2 pb-3 md:grid-cols-3">
            {workspaceRooms.slice(0, 3).map((room) => (
              <Link key={room.id} href={`/rooms/${room.id === "WARP-241" ? "preview-investor-qa" : "preview-partner-sync"}`} className="rounded-2xl border bg-white/80 p-3 transition hover:border-neutral-400">
        ...
        >
          <CardContent className="flex h-full items-center justify-between gap-3 p-4">
            <div>
              ...
              <p className="mt-1 text-2xl font-semibold">32 available</p>
