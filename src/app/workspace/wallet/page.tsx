...
export default function WorkspaceWalletPage() {
  ...
  const workspaceId =
    useWorkspaceStore((state) => state.activeWorkspaceId) || user?.id || "";
  const workspaceSlug =
    useWorkspaceStore((state) => state.activeWorkspaceSlug) || "";
  const workspaceName =
    useWorkspaceStore((state) => state.activeWorkspaceName) || "";
  ...
  const { data: historyPage, isLoading: isHistoryLoading } = useQuery({
    ...
    queryFn: () =>
      billingService.getCreditHistory(workspaceId, historyPageNumber, 20, {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage your credit balance, view history, and monitor AI usage.
          </p>
        </div>
                  ...
                  <TableBody>
                    {isHistoryLoading ? (
                      ...
                      groupedHistoryItems.map((tx, index) => {
                        ...
                        const rowIndex =
                          (historyPageNumber - 1) * 20 + index + 1;
                ...
                <p className="text-xs text-muted-foreground">
                  {historyPage ? (
                    <>
                      Showing{" "}
                      <strong>
                        {(historyPageNumber - 1) * 20 + 1}–
                        {Math.min(
                          historyPageNumber * 20,
                          historyPage.totalCount,
                        )}
                      </strong>{" "}
                      of <strong>{historyPage.totalCount}</strong> transactions
                    </>
                  ) : (
                    "Loading..."
                  )}
