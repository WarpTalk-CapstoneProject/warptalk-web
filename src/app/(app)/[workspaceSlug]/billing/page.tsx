...
function WorkspaceEnterpriseBillingContent() {
  ...
  const activeWorkspaceDefaultLanguage = useWorkspaceStore(
  ...
  const {
    ...
    isLoading: isWorkspaceLookupLoading,
    isError: isWorkspaceLookupError,
  } = useQuery({
  ...
  const routeWorkspace = useMemo(() => {
    ...
    workspaceLookup?.items,
  ]);

  const workspaceId =
    routeWorkspace?.id ??
    (!shouldResolveWorkspaceFromSlug ? activeWorkspaceId : "") ??
    "";
  const workspaceName =
    routeWorkspace?.name ??
    (!shouldResolveWorkspaceFromSlug ? activeWorkspaceName : null) ??
    "Workspace";
  const role = (
    routeWorkspace?.role ??
    (!shouldResolveWorkspaceFromSlug ? activeRole : null) ??
    ""
  ).toLowerCase();
  const isSystemBillingAdmin =
    user?.roles?.some((userRole) => userRole.toLowerCase() === "admin") ??
    false;
  const canViewBilling =
    role === "owner" || role === "admin" || isSystemBillingAdmin;
  ...
  const { data: salesInquiryPage, isLoading: isSalesInquiryLoading } = useQuery(
    {
      ...
      queryFn: () =>
        billingService.getWorkspaceSalesInquiries(workspaceId, 1, 5),
  ...
  const isTrial =
    hasTrialPeriod && new Date(subscription.trialEndsAt!).getTime() > nowMs;
  const isTrialEnded = hasTrialPeriod && !isTrial;
  const trialEndsAt = formatDate(subscription?.trialEndsAt);
  const currentCredits =
    balance?.currentCredits ?? subscription?.creditsRemaining ?? 0;
  const cycleCredits =
    subscription?.effectiveCreditsPerCycle ?? balance?.totalCredits ?? 0;
  const consumedCredits = Math.max(
    0,
    (balance?.totalCredits ?? cycleCredits) - currentCredits,
  );
  const usagePercent =
    cycleCredits > 0
      ? Math.min(100, Math.round((consumedCredits / cycleCredits) * 100))
      : 0;
  const invoiceTerms =
    subscription?.effectiveInvoiceTermsDays ??
    subscription?.invoiceTermsDaysOverride ??
    pricingConfig?.defaultInvoiceTermsDays ??
    0;
  const defaultPaymentTerms =
    invoiceTerms > 0 ? `Net ${invoiceTerms}` : "Net 15";
  const paymentTermOptions = useMemo(
    () => Array.from(new Set([defaultPaymentTerms, ...PAYMENT_TERM_OPTIONS])),
    [defaultPaymentTerms],
  );
  const extraUsageCap =
    subscription?.effectiveOverageCapCredits ??
    subscription?.overageCapCreditsOverride ??
    0;
  const invoices = invoicesPage?.items ?? [];
  const latestContractInquiry =
    salesInquiryPage?.items?.find(
      (inquiry) => inquiry.requestType === "enterprise_contract_request",
    ) ?? null;
  const hasPendingContractInquiry = latestContractInquiry
    ? !["converted", "closed"].includes(
        latestContractInquiry.status.toLowerCase(),
      )
    : false;
  const shouldShowNoBillingPlan =
    !!workspaceId && !isSubscriptionLoading && !hasSubscription;
  const workspaceDefaultLanguage =
    routeWorkspace?.defaultLanguage ?? activeWorkspaceDefaultLanguage ?? "en";
  ...
  const createInvoiceCheckoutMutation = useMutation({
    mutationFn: (invoice: InvoiceDto) =>
      billingService.createInvoiceCheckout(invoice.id),
    onSuccess: ({ url }) => {
  ...
  const requestContractMutation = useMutation({
    mutationFn: () => {
      ...
      const requestedMonthlyCredits =
        contractRequest.requestedMonthlyCredits.trim();
      ...
      const requestedWorkspaceMembers =
        contractRequest.requestedWorkspaceMembers.trim();
      ...
      const requestedLanguageNames = requestedLanguages
        .map(getLanguageName)
        .join(", ");
      ...
      const featureInterests = [
        ...
        "workspace_trial",
        "ai_meetings",
        ...(requestedAiServices.includes(GOOGLE_MEET_INTEGRATION_LABEL)
          ? ["google_meet"]
          : []),
      ];
    ...
    onError: (err: unknown) => {
      toast.error(
        getErrorMessage(err, "Could not send Enterprise contract request"),
      );
    },
  });
  ...
  const handleRequestContract = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const requestedMonthlyCredits = Number(
      contractRequest.requestedMonthlyCredits,
    );
    if (
      !Number.isInteger(requestedMonthlyCredits) ||
      requestedMonthlyCredits < 1
    ) {
    ...
    if (requestedMonthlyCredits > MAX_REQUESTED_MONTHLY_CREDITS) {
      toast.error(
        `Monthly credits cannot exceed ${formatNumber(MAX_REQUESTED_MONTHLY_CREDITS)}`,
      );
      return;
    }

    const requestedWorkspaceMembers = Number(
      contractRequest.requestedWorkspaceMembers,
    );
    if (
      !Number.isInteger(requestedWorkspaceMembers) ||
      requestedWorkspaceMembers < 1
    ) {
    ...
    if (requestedWorkspaceMembers > MAX_REQUESTED_WORKSPACE_MEMBERS) {
      toast.error(
        `Workspace members cannot exceed ${formatNumber(MAX_REQUESTED_WORKSPACE_MEMBERS)}`,
      );
      return;
    }
    ...
    if (contractRequest.requestedLanguages.length > MAX_ENTERPRISE_LANGUAGES) {
      toast.error(
        `Select up to ${MAX_ENTERPRISE_LANGUAGES} supported languages`,
      );
      return;
    }
      ...
      : subscription?.suspendedReason === "overage_cap"
        ...
        : subscription?.suspendedReason === "invoice_overdue"
          ...
          : subscription?.suspendedReason ||
            "Resolve the billing status with WarpTalk billing to resume AI usage.";
  ...
  if (
    ...
    (workspaceLookupTimedOut ||
      isWorkspaceLookupError ||
      (!isWorkspaceLookupLoading && !routeWorkspace))
  ) {
    return (
      <div className="flex h-full items-center justify-center bg-canvas p-6">
        <Card className="max-w-md rounded-md border-border">
          <CardHeader>
            <CardTitle className="text-base">
              Workspace billing unavailable
            </CardTitle>
            <CardDescription>
              This workspace was not found or your account does not have access
              to its billing page.
            </CardDescription>
          </CardHeader>
        </Card>
      ...
      <div className="flex h-full items-center justify-center bg-canvas p-6">
        <Card className="max-w-md rounded-md border-border">
          <CardHeader>
            ...
            <CardDescription>
              Only workspace Owners and Administrators can view Enterprise trial
              and invoice status.
            </CardDescription>
          </CardHeader>
        </Card>
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
            ...
            <div>
              <div className="mb-2 flex flex-wrap items-center gap-2">
                {!shouldShowNoBillingPlan && (
                  <Badge
                    variant={
                      statusTone as "default" | "secondary" | "destructive"
                    }
                  >
                    {subscription?.serviceState === "suspended"
                      ? "Suspended"
              ...
              <p className="mt-1 max-w-2xl text-sm text-ink-muted">
                Review trial access, contract status, open invoices, and
                Enterprise usage for this workspace.
              </p>
            </div>
          </div>
        </header>
          ...
          <section className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(360px,0.85fr)]">
            <Card className="rounded-md border-border">
              <CardHeader>
                ...
                <CardDescription>
                  Start the self-service Enterprise trial for this workspace.
                  Approved contract terms and invoices are handled later by
                  WarpTalk billing.
                </CardDescription>
              </CardHeader>
              <CardContent className="flex flex-col gap-5">
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <StatusRow
                    label="Trial credits"
                    value="Granted when trial starts"
                  />
                  <StatusRow
                    label="Workspace seats"
                    value={`Up to ${TRIAL_WORKSPACE_MEMBER_LIMIT} members`}
                  />
                  <StatusRow
                    label="Extra usage"
                    value="Disabled during trial"
                  />
                  <StatusRow
                    label="Contract flow"
                    value="Admin creates approved terms"
                  />
                </div>
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    disabled={
                      startTrialMutation.isPending || !user?.id || !user.email
                    }
            ...
            <Card className="rounded-md border-border">
              <CardHeader>
                ...
                <CardDescription>
                  Trial usage stays separate from the paid contract. Finance
                  only receives invoices after an Enterprise contract is
                  approved and a billing cycle closes.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3 text-sm text-ink-muted">
                <div className="rounded-md border border-border bg-surface-1 p-3">
                  Trial gives the team enough credits to test meetings,
                  transcripts, and AI summaries without extra usage charges.
                </div>
                <div className="rounded-md border border-border bg-surface-1 p-3">
                  After admin approval, WarpTalk saves the negotiated credits,
                  price before VAT, overage cap, and NET terms.
                </div>
                <div className="rounded-md border border-border bg-surface-1 p-3">
                  Open invoices can be paid online by the workspace owner after
        ...
        {!shouldShowNoBillingPlan && (
          <>
                ...
                <CardContent className="flex items-start gap-3 p-4">
                  ...
                  <div>
                    <div className="text-sm font-medium text-destructive">
                      AI service is currently suspended
                    </div>
            ...
            <section
              className={`grid gap-4 md:grid-cols-2 ${hasTrialPeriod ? "xl:grid-cols-3" : "xl:grid-cols-4"}`}
            >
              <MetricCard
                icon={Coins}
                ...
                value={
                  isLoading
                    ? "Loading..."
                    : new Intl.NumberFormat("vi-VN").format(currentCredits)
                }
              ...
              <MetricCard
                ...
                detail={
                  hasTrialPeriod
                    ? "Trial does not create invoices or extra usage"
                    : invoiceTerms > 0
                      ? `Net ${invoiceTerms} invoice terms`
                      : "Configured invoice terms"
                }
              />
              <MetricCard
                icon={Buildings}
            ...
            <Card className="rounded-md border-border">
              <CardHeader className="pb-3">
                <CardTitle className="text-base">
                  {hasTrialPeriod ? "Trial status" : "Contract status"}
                </CardTitle>
                <CardDescription>
                  {hasTrialPeriod
              ...
              <CardContent className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
                ...
                <StatusRow
                  ...
                  value={
                    subscription?.billingContactEmail || "billing@warptalk.com"
                  }
                />
                {hasTrialPeriod ? (
                  <>
                  ...
                  <>
                    <StatusRow
                      ...
                      value={formatVnd(
                        subscription?.effectiveContractPriceVnd ??
                          subscription?.contractPriceVnd,
                      )}
                    />
                    <StatusRow
                      label="Invoice terms"
                      value={
                        invoiceTerms > 0 ? `Net ${invoiceTerms}` : "Configured"
                      }
                    />
                  </>
                )}
              ...
              <Card className="rounded-md border-border">
                ...
                <CardContent>
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    ...
                    />
                    <StatusRow
                      label={hasTrialPeriod ? "Trial total" : "Cycle total"}
                      value={new Intl.NumberFormat("vi-VN").format(
                        cycleCredits,
                      )}
                    />
              ...
              {hasTrialPeriod ? (
                  ...
                  <CardContent className="grid gap-3">
                    {hasPendingContractInquiry ? (
                      <StatusRow
                        ...
                        value={
                          latestContractInquiry?.status || "Pending review"
                        }
                      />
                    ) : null}
                    <Button
                      ...
                      className="w-full"
                      disabled={
                        hasPendingContractInquiry || isSalesInquiryLoading
                      }
                ...
                <Card className="rounded-md border-border">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Pay open Enterprise invoices online when the billing cycle
                      closes.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    {isInvoicesLoading ? (
                      ...
                      <div className="rounded-md border border-dashed border-border p-4 text-sm text-ink-muted">
                        No invoices yet. Usage will stay visible here until an
                        Enterprise billing cycle closes.
                      </div>
                    ) : (
                            ...
                            <div className="flex shrink-0 items-center gap-3 text-right">
                              {isInvoiceOpen(invoice) && (
                                <button
                                  type="button"
                                  onClick={() =>
                                    createInvoiceCheckoutMutation.mutate(
                                      invoice,
                                    )
                                  }
                                  disabled={
                                    createInvoiceCheckoutMutation.isPending
                                  }
            ...
            {!hasTrialPeriod && (
              <section className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(360px,0.85fr)]">
                <UsageChart
                  ...
                  refetchIntervalMs={BILLING_REALTIME_REFETCH_MS}
                />
            ...
            <Dialog
              open={isContractRequestOpen}
              onOpenChange={setIsContractRequestOpen}
            >
              <DialogContent className="max-h-[90vh] overflow-y-auto border-border bg-surface-1 text-ink sm:max-w-[720px]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Enter the company, invoice, usage, and plan details needed
                    to prepare the subscription contract.
                  </DialogDescription>
                </DialogHeader>
                ...
                <form className="grid gap-4" onSubmit={handleRequestContract}>
                  <div className="grid gap-3 sm:grid-cols-2">
                      ...
                      <Input
                        ...
                        onChange={(event) =>
                          setContractRequest((current) => ({
                            ...
                            invoiceEmail:
                              current.invoiceEmail || event.target.value,
                          }))
                        }
                    ...
                    <label className="grid gap-1.5 text-sm font-medium text-ink">
                      ...
                      <Select
                        value={
                          contractRequest.paymentTerms || defaultPaymentTerms
                        }
                        onValueChange={(value) =>
                          setContractRequest((current) => ({
                            ...current,
                        ...
                        <div className="grid gap-2 sm:grid-cols-2">
                          {CONTRACT_LANGUAGE_OPTIONS.map((language) => (
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={contractRequest.requestedLanguages.includes(
                                  language.code,
                                )}
                                onChange={() =>
                                  setContractRequest((current) => {
                                    const isSelected =
                                      current.requestedLanguages.includes(
                                        language.code,
                                      );
                                    if (
                                      !isSelected &&
                                      current.requestedLanguages.length >=
                                        MAX_ENTERPRISE_LANGUAGES
                                    ) {
                                      toast.error(
                                        `Select up to ${MAX_ENTERPRISE_LANGUAGES} supported languages`,
                                      );
                                    ...
                                    return {
                                      ...current,
                                      requestedLanguages: toggleStringValue(
                                        current.requestedLanguages,
                                        language.code,
                                      ),
                                    };
                                  })
                        ...
                        <div className="grid gap-2 sm:grid-cols-2">
                          {CONTRACT_AI_SERVICE_OPTIONS.map((option) => (
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={contractRequest.requestedAiServices.includes(
                                  option,
                                )}
                                onChange={() =>
                                  setContractRequest((current) => ({
                                    ...current,
                                    requestedAiServices: toggleStringValue(
                                      current.requestedAiServices,
                                      option,
                                    ),
                                  }))
                                }
                  ...
                  <div className="flex justify-end gap-2 border-t border-border pt-4">
                    ...
                    <Button
                      type="submit"
                      disabled={
                        requestContractMutation.isPending || !user?.email
                      }
                    >
