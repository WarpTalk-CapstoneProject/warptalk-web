...
export default function WorkspaceAccessManagementPage() {
  ...
  return <div className="space-y-6 p-6">
    ...
    {receipt && <Card><CardHeader><CardTitle>Latest role-change receipt</CardTitle></CardHeader><CardContent className="space-y-1 text-sm"><p>{receipt.oldRole} → {receipt.newRole}</p><p>Effective: {new Date(receipt.effectiveAt).toLocaleString()} for the next request/session.</p><p className="text-xs text-muted-foreground">Event ID: {receipt.auditId}. Durable role-change history is not stored by Workspace in this schema-free flow.</p></CardContent></Card>}
  </div>;
}
