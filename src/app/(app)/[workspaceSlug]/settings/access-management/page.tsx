...
export default function WorkspaceAccessManagementPage() {
  ...
  const openPreview = async (userId: string, targetRole: "Admin" | "Member") => {
    ...
    try {
      const result = await preview.mutateAsync({ userId, toRole: targetRole });
  ...
  return <div className="space-y-6 p-6">
    ...
    <Card><CardHeader><CardTitle>Active internal members</CardTitle></CardHeader><CardContent className="space-y-3">
      {members.filter((m) => m.membershipType.toLowerCase() === "internal" && normalizeWorkspaceRole(m.roleName) !== "owner").map((member) => { const memberRole = normalizeWorkspaceRole(member.roleName); return <div key={member.userId} className="flex items-center justify-between rounded border p-3"><div><div className="font-medium">{member.fullName}</div><div className="text-sm text-muted-foreground">{member.email} · {member.roleName} · Can create meetings: {member.canCreateMeetings ? "Yes" : "No"}</div></div><div className="flex gap-2">{memberRole === "member" && <Button variant="outline" onClick={() => openPreview(member.userId, "Admin")}>Preview promote</Button>}{memberRole === "admin" && <Button variant="outline" onClick={() => openPreview(member.userId, "Member")}>Preview demote</Button>}</div></div>; })}
    </CardContent></Card>
    {selected && target && <Card><CardHeader><CardTitle>Review role change</CardTitle></CardHeader><CardContent className="space-y-3"><p>{target.fullName}: {target.roleName} → {toRole}</p><p className="text-sm">{toRole === "Admin" ? "Promotion has a 60-second cooling-off before final confirmation." : "Demotion applies to the next request/session."}</p><p className="text-xs text-muted-foreground">Preview expires {preview.data?.expiresAt ? new Date(preview.data.expiresAt).toLocaleString() : "soon"}.</p><ul className="list-disc pl-5 text-sm">{(preview.data?.impact ?? []).map((item: string) => <li key={item}>{item}</li>)}</ul><Input value={confirmation} onChange={(e) => setConfirmation(e.target.value)} placeholder={`Type ${target.email || target.fullName} to confirm`} /><div className="flex gap-2"><Button onClick={() => { if (toRole === "Admin" && !promotionReady) { if (!cooldownUntil) setCooldownUntil(Date.now() + 60000); } else apply(); }} disabled={change.isPending || confirmation.trim().toLowerCase() !== (target.email || target.fullName).trim().toLowerCase()}>{toRole === "Admin" && !promotionReady ? (remaining ? `Wait ${remaining}s` : "Start 60-second review") : "Confirm role change"}</Button><Button variant="ghost" onClick={() => { setSelected(null); setPromotionReady(false); setCooldownUntil(null); }}>Cancel</Button></div></CardContent></Card>}
  </div>;
}
