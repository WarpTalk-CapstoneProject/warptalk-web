...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  }, [activeWorkspaceId, activeWorkspaceSlug, workspacesData, workspacesLoading, isOnboardingRoute, routeWorkspaceSlug, isAdminRoute, selectWorkspace, setActiveWorkspace, router, mounted, isAuthenticated]);
  ...
  if (!isAdminRoute && ((!activeWorkspaceId && !routeWorkspaceSlug) || workspacesLoading)) {
    return (
      <div className="flex h-dvh w-screen items-center justify-center bg-canvas">
        <Spinner className="h-6 w-6 animate-spin text-ink-muted" />
      </div>
