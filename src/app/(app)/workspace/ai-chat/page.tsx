...
export default function WorkspaceAiChatPage() {
  ...
  return (
    <div className="grid h-full min-h-[620px] grid-cols-[280px_minmax(0,1fr)] gap-3 pb-2">
      <Card className="flex min-h-0 flex-col overflow-hidden rounded-3xl border-hairline bg-surface-1 text-ink shadow-sm">
        <CardHeader className="border-b p-4">
          <div className="flex items-center justify-between">
            ...
            <Button size="icon-sm" className="rounded-full bg-primary text-primary-foreground hover:bg-primary-hover" onClick={createConversation}><ChatCircleText weight="light" /></Button>
          </div>
          <div className="relative pt-2">
            ...
            <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="MagnifyingGlass conversations..." className="h-9 rounded-xl bg-surface-2 border-hairline text-ink pl-9" />
          </div>
        </CardHeader>
        <CardContent className="min-h-0 flex-1 space-y-2 overflow-y-auto p-3">
          {visible.map((conversation) => (
            <div key={conversation.id} className={`group flex items-center gap-2 rounded-2xl border p-2 transition ${activeId === conversation.id ? "border-primary bg-primary text-primary-foreground" : "border-hairline bg-surface-2 text-ink hover:border-hairline-strong"}`}>
              <button onClick={() => setActiveId(conversation.id)} className="min-w-0 flex-1 p-1 text-left">
                <p className="truncate text-sm font-medium">{conversation.title}</p>
                <p className={`truncate text-xs ${activeId === conversation.id ? "text-primary-foreground/80" : "text-ink-muted"}`}>{conversation.context}</p>
              </button>
              <Button variant="ghost" size="icon-sm" className={activeId === conversation.id ? "text-primary-foreground/90 hover:bg-primary-foreground/10 hover:text-primary-foreground" : ""} onClick={() => removeConversation(conversation.id)} title="Delete conversation"><Trash weight="light" /></Button>
            </div>
          ))}
        </CardContent>
      </Card>
            ...
            <CardContent className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5">
              {active.messages.map((item, index) => (
                <div key={`${item.role}-${index}`} className={`flex items-start gap-2 ${item.role === "user" ? "justify-end" : ""}`}>
                  {item.role === "assistant" && <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-surface-2 border border-hairline text-ink"><Robot weight="light" className="h-4 w-4" /></span>}
                  <div className={`max-w-[76%] rounded-2xl px-4 py-3 text-sm leading-6 ${item.role === "user" ? "bg-primary text-primary-foreground" : "border border-hairline bg-surface-2 text-ink"}`}>{item.text}</div>
                </div>
              ))}
            </CardContent>
            <div className="border-t p-4">
              <div className="mb-2 flex items-center gap-2 rounded-xl bg-surface-2 border border-hairline px-3 py-2 text-xs text-ink-muted"><FileText weight="light" className="h-4 w-4" />{active.context} transcript attached</div>
              <div className="flex items-center gap-2 rounded-2xl border border-hairline bg-surface-2 p-2">
                ...
                <Input value={message} onChange={(event) => setMessage(event.target.value)} onKeyDown={(event) => event.key === "Enter" && sendMessage()} placeholder="Ask about meetings, transcripts, costs, or teams..." className="h-9 flex-1 border-0 bg-transparent text-ink shadow-none focus-visible:ring-0" />
                <Button size="icon-sm" className="rounded-xl bg-primary text-primary-foreground hover:bg-primary-hover" onClick={sendMessage}><PaperPlaneRight weight="light" /></Button>
              </div>
            </div>
          </>
