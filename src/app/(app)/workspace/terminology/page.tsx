...
const glossarySchema = z.object({
  ...
  businessDomain: z.string().min(2, "Domain must be at least 2 characters (e.g. Legal, Finance)"),
...
const termSchema = z.object({
  ...
  preferredTranslation: z.string().min(1, "Translation cannot be empty"),
...
export default function WorkspaceTerminologyPage() {
  ...
  const handleAddTerm = async (data: TermFormData) => {
    ...
    const isDuplicate = existingTerms.some(
      (t) => t.term.toLowerCase() === data.term.toLowerCase()
    );

    if (isDuplicate) {
      toast.error(`The term "${data.term}" already exists in this glossary.`);
      return;
    }
    ...
    try {
      ...
      toast.success(`Term "${data.term}" added.`);
      resetTerm();
  ...
  const handleCSVImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        // Skip duplicates
        const isDuplicate = existingTerms.some((t) => t.term.toLowerCase() === term.toLowerCase());
        if (isDuplicate) {
          dupCount++;
          continue;
        }
  ...
  const handleCSVExport = () => {
    ...
    const rows = terms
      .map(
        (t) =>
          `"${t.term.replace(/"/g, '""')}","${t.preferredTranslation.replace(/"/g, '""')}","${(
            t.definition || ""
            ...
            <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-4 border-b border-hairline pb-3">
              <div>
                ...
                <CardDescription className="text-xs">
                  Domain: {selectedGlossary.businessDomain} • Pair: {selectedGlossary.sourceLanguage.toUpperCase()} → {selectedGlossary.targetLanguage.toUpperCase()}
                </CardDescription>
              </div>
          ...
          <form onSubmit={handleGlossarySubmit(handleCreateGlossary)} className="flex flex-col gap-3 my-2">
            ...
            <div className="flex flex-col gap-1">
              ...
              <Input
                ...
                className="h-8 border-hairline text-xs"
                {...registerGlossary("businessDomain")}
              />
              {glossaryErrors.businessDomain && (
                <p className="text-[10px] text-destructive">{glossaryErrors.businessDomain.message}</p>
              )}
            </div>
        ...
        <DialogContent className="border-hairline bg-surface-1 max-w-sm">
          ...
          <form onSubmit={handleTermSubmit(handleAddTerm)} className="flex flex-col gap-3 my-2">
            <div className="flex flex-col gap-1">
              ...
              />
              {termErrors.term && (
                <p className="text-[10px] text-destructive">{termErrors.term.message}</p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              <Input
                ...
                className="h-8 border-hairline text-xs"
                {...registerTerm("preferredTranslation")}
              />
              {termErrors.preferredTranslation && (
                <p className="text-[10px] text-destructive">{termErrors.preferredTranslation.message}</p>
              )}
            </div>
